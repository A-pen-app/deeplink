...
func NewRecruitNewPostLink(platform Platform, postID string) Deeplink {
	p := RecruitNewPostLink{
		platform: platform,
		postID:   postID,
	}
	return &p
...
type RecruitNewPostLink struct {
	platform Platform
	postID   string
}

func (p *RecruitNewPostLink) Build() (string, error) {
	...
	params.Add("c", string(RecruitNewPostCampaign))
